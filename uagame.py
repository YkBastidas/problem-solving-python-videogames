...
from pygame import (init, Color, Surface, KEYUP, K_SPACE, K_RETURN, K_z,
                    K_LSHIFT, K_RSHIFT, K_CAPSLOCK, K_BACKSPACE)
from pygame import quit as quit_
...
class Window:
    ...
    def draw_string(self, string, x, y):
        ...
        text_image = self.__font__.render(string, True, Color(
            self.__font_color__), Color(self.__bg_color__))
        ...
        while key != K_RETURN:
            ...
            if (K_SPACE <= key <= K_z):
                if key == K_SPACE:
                    ...
                    letter = name(key)
                if key_state[K_LSHIFT] or key_state[K_RSHIFT] or (
                        key_state[K_CAPSLOCK]):
                    letter = letter.upper()
...
def _test():
    ...
    height = window.get_font_height()
    window.input_string('press any key to close window',
                        0, window_height - height)
    window.close()
